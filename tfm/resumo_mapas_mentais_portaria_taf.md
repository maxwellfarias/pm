...
mindmap
  root((TAF PMAL))
    Finalidade
      Regular aplicação e execução
      Melhorar saúde
      Melhorar desempenho
      Viabilizar ascensão na carreira
    Tipos
      TAF I
        Condição normal de saúde
      TAF II
        Gestante
        Puerperal até 6 meses
        Restrição médica temporária sem LTS
      TAF III
        Candidato a ingresso
      TAF IV
        Idoso
      TAF V
        Curso operacional específico
    Exercícios
      Abdominal
      Barra fixa
      Apoio de frente
...
```mermaid
mindmap
  root((Composição do TAF))
    Abdomen
      Flexão abdominal supra
      Pés no chão e cotovelos nos quadríceps
      Pés no banco com elevação das escápulas
      Mede resistência abdominal
    Membros superiores
      Barra fixa
        Masculino
...
```mermaid
mindmap
  root((Aplicação do TAF))
    Antes
      Inspeção de Saúde precede o TAF
      Locais e horários na convocação
      Preferência pela área onde o PM trabalha
    Apoio
      Apoio médico é essencial
      DEIP solicita equipe médica
    Comissão Examinadora
      Nomeada pelo Diretor de Ensino
      Presidente
      ...
      Membros efetivos
        Até 30 PMs
      Membros suplentes
        Demais PMs com cursos ou estágios relacionados
    Banca Examinadora
      Mínimo 3 membros
      1 examinador para até 15 examinados
      Presidente deve ser oficial superior ou mais antigo
    Inteligência
      Pode usar trajes civis se autorizado
      DINT confere
      Requerimento ao DEFD em até 24h
```
...
```mermaid
flowchart TD
    A[Convocação] --> B[Locais e horários divulgados]
    B --> C[Inspeção de Saúde]
    C --> D{Apto na IS?}
    D -- Não --> E[Não realiza o TAF]
    D -- Sim --> F[Questionário PRÉ-TAF quando aplicável]
    F --> G{Respondeu SIM?}
    G -- Sim --> H[Nova inspeção de saúde]
    G -- Não --> I[Realiza o TAF]
```
...
```mermaid
flowchart TD
    A[Resultado APTO] --> B{Qual TAF?}
    B -- I, IV ou V --> C[Pode ter validade de 180 dias]
    B -- II --> D[Validade vedada]
    C --> E{Edital classificatório?}
    E -- Sim --> F[Convalidação vedada]
    E -- Não --> G[Usa para matrícula conforme regras]
```
